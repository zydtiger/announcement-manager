...
$(function () {
  if ($('#login-err-msg').text()) $('#login-err-msg').show();else $('#login-err-msg').hide();
  if ($('#signup-err-msg').text()) $('#signup-err-msg').show();else $('#signup-err-msg').hide();
  $('#signup').hide();
  if ($('#pipeline').text() == 'signup') switch_block(); // hide the eye indicator

  $('i.fa').hide();
  $('#signup-uname-cnt, #signup-pwd-cnt, #login-uname-cnt, #login-pwd-cnt').hide();
  $('#login-pwd').on('input', function () {
    return pwd_input('#login-pwd');
  });
  $('#login-uname').on('input', function () {
    return uname_input('#login-uname');
  });
  $('#signup-pwd').on('input', function () {
    return pwd_input('#signup-pwd');
  });
  $('#signup-uname').on('input', function () {
    return uname_input('#signup-uname');
  });
  $('#signup-pwd-1').on('input', function () {
    return pwd_input('#signup-pwd-1');
  });
});
...
function signup_submit() {
  ...
  var pwd_confirm = $('#signup-pwd-1').val(); // console.log(pwd, pwd_confirm)
  ...
  if (!uname) {
    ...
    if (pwd === pwd_confirm) document.getElementById('signup-form').submit();else {
