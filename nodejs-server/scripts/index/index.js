$(() => {
    ...
    let announcements = new Vue({
        ...
        methods: {
            chg_content: function (target) {
                $('iframe').attr('src', `/announce?target=${target}`)
                if (is_small_screen()) hide_menu()
            },
            chg_content_url: function (url) {
                // $('iframe').attr('src', url)
                location.href = url
                // if (is_small_screen()) hide_menu()
            },
            expand: function (obj, evt) {
                let target = $(evt.target)[0].tagName === 'SPAN' ? $(evt.target).parent() : $(evt.target)
...
function toggle_menu() {
    let toggle_class = $('#toggle-menu .fa').attr('class')
